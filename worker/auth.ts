 ...
 */
export async function createSessionToken(_password: string, envSecret?: string): Promise<string> {
	...
	const key = await crypto.subtle.importKey(
		'raw',
		encoder.encode(getSessionSecret(envSecret)),
...
export async function verifySessionToken(token: string, envSecret?: string): Promise<boolean> {
	try {
