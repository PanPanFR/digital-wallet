 ...
 * All functions take a D1Database as the first arg; callers in the Hono
 * worker pass `c.env.DB`.
 */
...
export async function listTransactions(
	...
	const { limit = 100, offset = 0, month, walletId, kind, search, category } = opts;
	const where: string[] = [];
	const binds: unknown[] = [];
