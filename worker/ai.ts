...
export function getConfigFromEnv(
	apiKey: string,
	env?: { AI_BASE_URL?: string; AI_MODEL?: string }
): AiConfig {
	return {
		baseUrl: (env?.AI_BASE_URL || process.env.AI_BASE_URL || DEFAULT_BASE_URL).replace(/\/$/, ''),
		apiKey,
		model: env?.AI_MODEL || process.env.AI_MODEL || DEFAULT_MODEL
	};
}
