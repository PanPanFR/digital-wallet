...
| `src/lib/server/db.test.ts` | wallet CRUD (duplicate-name guard, balance adjustment), transaction filters/bulk delete, debt CRUD + payment batching/overpay guard, computed balances (empty wallet = 0, per-kind subtotals) |
