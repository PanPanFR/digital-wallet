...
| `GOOGLE_API_KEY` | secret | **yes** | Bearer token for the AI copilot's OpenAI-compatible endpoint (name kept for old-app compatibility). Optional: without it `/api/ai/*` return 503 and the copilot shows "AI belum dikonfigurasi" | `src/routes/api/ai/parse/+server.ts:17`, `report/+server.ts:17` |
| `AI_BASE_URL` | plain var | no | Chat-completions base URL. Default `https://9router.panpan.my.id/v1` (`ai.ts:21`) | `src/lib/server/ai.ts:32` |
| `AI_MODEL` | plain var | no | Model name. Default `gemini-2.5-flash` (`ai.ts:22`) | `src/lib/server/ai.ts:34` |
...
| `src/lib/server/db.test.ts` | wallet CRUD, computed balances (empty wallet = 0, per-kind subtotals) |
| `src/lib/server/validation.test.ts` | `TxSchema`/`WalletSchema` money-path rules |
| `src/lib/server/ai.test.ts` | parse-output normalization, loose-JSON extraction |
...
| Copilot says "Fitur AI belum dikonfigurasi" (503) | No `GOOGLE_API_KEY` in `.dev.vars` (local) or Worker secrets (prod). Expected degradation, not a crash. |
