...
| Secrets | `SESSION_SECRET` (required), `GOOGLE_API_KEY` (optional — env fallback for the copilot when no provider is stored in `/settings`) | set once, persist across deploys |
| Plain vars | `AI_BASE_URL`, `AI_MODEL` — only to override the env-fallback defaults | dashboard Variables, or omit. (In-app AI providers live in D1 `app_settings`, configured on `/settings`, not here.) |

Secrets (either path; they target the deployed Worker, not local dev):

```bash
npx wrangler secret put SESSION_SECRET
npx wrangler secret put GOOGLE_API_KEY   # optional; copilot env fallback
```
...
**Structural changes** (new column, changed CHECK constraint) can't ride on `CREATE TABLE IF NOT EXISTS` — they need a dedicated migration script that rebuilds the table and copies rows, committed under `migrations/NNN-*.sql` for the record. Applied examples: `migrations/001-tx-date-transfer.sql` (transactions rebuild adding `date`/`to_wallet_id`/`transfer`, row-preserving) and `migrations/002-debts.sql` (additive `debts`/`debt_payments`). Run the script `--local` **and** `--remote` **before** merging the code that expects the new shape, and smoke-test prod data after. Production rows are user data; the app is single-user, so a `--remote --command "SELECT COUNT(*) FROM transactions"` before/after is a cheap sanity check.
...
3. With `GOOGLE_API_KEY` unset and no provider stored in `/settings` → `/copilot` chat returns "Fitur AI belum dikonfigurasi", nothing crashes.
