...
import { getCategoryTotals, getMonthlyTotals, getWalletTotals } from '$lib/server/db';
...
export const load: ServerLoad = async ({ locals, platform, url }: RequestEvent) => {
	...
	const [categoryTotals, monthlyTotals, walletTotals] = await Promise.all([
		getCategoryTotals(db, month),
		getMonthlyTotals(db, 6),
		getWalletTotals(db, month)
	]);
	return { month, categoryTotals, monthlyTotals, walletTotals };
};
